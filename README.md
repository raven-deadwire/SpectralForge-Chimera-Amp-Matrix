...
It works as a conventional single-amp simulator in **Classic Mode**, blends two complete amp rigs in **Dual Mode**, or splits the instrument signal into multiple frequency bands in **Matrix Mode**, keeping a clean compressed LOW DI foundation under independent MID/HIGH amp and cabinet rigs.
...
* **Dual** — Blend two independent guitar/bass amp rigs
...
Implemented signal path: **input gain -> gate -> transpose -> pre drive -> rigs/cabinets -> merge -> delay -> reverb -> output gain**. The chromatic tuner taps the input before the gate and transpose; optional auto-mute silences the output while tuning.

- Eight independently voiced amp algorithms with 1x/2x/4x/8x oversampling (4x default).
- Matrix LOW: clean DI, one-knob VCA-style COMP, Level and Band Tone. Pre Drive, amp and IR are bypassed on LOW with matching processing delay. MID/HIGH retain Drive, Level and Band Tone. Classic/Dual retain full-range EQ.
- PRE pedalboard: Gate, Transpose, Tight Drive. POST rack: Stereo Delay, Room Reverb. Each is a real bypassable module with fixed mode responsibility.
