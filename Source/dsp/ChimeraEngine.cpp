...
void ChimeraEngine::process(juce::AudioBuffer<float>&b,RoutingMode mode,float x1,float x2,const std::array<LaneSettings,3>&s){
 ...
 for(int i=0;i<3;++i){work[i].setSize(nc,ns,false,false,true);lanes[i].setModel((AmpModel)juce::jlimit(0,3,s[i].model));lanes[i].setLevelDb(s[i].levelDb);lanes[i].setTone(s[i].bass,s[i].mid,s[i].treble,s[i].presence,s[i].resonance);lanes[i].setCabEnabled(s[i].cabEnabled);lanes[i].setCabCuts(s[i].cabLowCut,s[i].cabHighCut);lanes[i].setMuted(s[i].mute||(any&&!s[i].solo));}
