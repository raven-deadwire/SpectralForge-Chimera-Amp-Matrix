...
void ChimeraEngine::applyAlignment(juce::AudioBuffer<float>&b,int lane,const LaneSettings&s){
 const float maxMs=5.f, ms=juce::jlimit(0.f,maxMs,s.fineDelayMs);const float automatic=juce::jmax(0.f,alignmentReferenceSamples-lanes[(size_t)lane].getLatencySamples());delays[(size_t)lane].setDelay(automatic+(float)(sampleRate*ms/1000.0));
...
void ChimeraEngine::process(juce::AudioBuffer<float>&b,RoutingMode mode,float x1,float x2,const std::array<LaneSettings,3>&s){
 int ns=b.getNumSamples(),nc=b.getNumChannels();alignmentReferenceSamples=0.f;for(const auto& lane:lanes)alignmentReferenceSamples=juce::jmax(alignmentReferenceSamples,lane.getLatencySamples());bool any=s[0].solo||s[1].solo||s[2].solo;
 for(int i=0;i<3;++i){work[i].setSize(nc,ns,false,false,true);lanes[i].setModel((AmpModel)juce::jlimit(0,3,s[i].model));lanes[i].setDrive(s[i].drive);lanes[i].setLevelDb(s[i].levelDb);lanes[i].setTone(s[i].bass,s[i].mid,s[i].treble,s[i].presence,s[i].resonance);lanes[i].setCabEnabled(s[i].cabEnabled);lanes[i].setCabCuts(s[i].cabLowCut,s[i].cabHighCut);lanes[i].setMuted(s[i].mute||(any&&!s[i].solo));}
