...
void PostFxRack::process(juce::AudioBuffer<float>&b){if(chorusOn){dry.setSize(b.getNumChannels(),b.getNumSamples(),false,false,true);for(int ch=0;ch<b.getNumChannels();++ch)dry.copyFrom(ch,0,b,ch,0,b.getNumSamples());juce::dsp::AudioBlock<float>bl(b);juce::dsp::ProcessContextReplacing<float>ctx(bl);chorus.process(ctx);for(int ch=0;ch<b.getNumChannels();++ch){b.applyGain(ch,0,b.getNumSamples(),chorusMix);b.addFrom(ch,0,dry,ch,0,b.getNumSamples(),1.f-chorusMix);}}if(limiterOn){juce::dsp::AudioBlock<float>bl(b);juce::dsp::ProcessContextReplacing<float>ctx(bl);limiter.process(ctx);}}
}
