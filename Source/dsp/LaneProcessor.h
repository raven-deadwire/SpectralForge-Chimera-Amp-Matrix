...
class LaneProcessor {
public:
 void prepare(const juce::dsp::ProcessSpec& s){ level.reset(s.sampleRate,0.02); level.setCurrentAndTargetValue(1.0f); }
 void reset(){}
...
private: AmpModel model{AmpModel::clean}; float drive{0.35f}; bool muted{}; juce::SmoothedValue<float> level{1.f};
}; }
